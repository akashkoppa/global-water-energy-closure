...
# normalize the dist metrics by ai
...
# remove super anamolous values
dist_metric[dist_metric > 1000] = NA
dist_metric[dist_metric > 5] = 5
...
rmse_aridity = data.frame(Aridity = rev(c("Hyper-arid","Arid","Semi-arid","Dry sub-humid","Humid")))
temp = as.data.frame(cbind(rmse_humid,rmse_drysubhumid,rmse_semiarid,rmse_arid,rmse_hyperarid))
...
colnames(p_rmse_aridity) =c("P_Dataset", rev(c("Hyper-arid","Arid","Semi-arid","Dry sub-humid","Humid")))
...
colnames(et_rmse_aridity) = c("ET_Dataset",rev(c("Hyper-arid","Arid","Semi-arid","Dry sub-humid","Humid")))
...
p_rmse_aridity_rank = p_rmse_aridity
p_rmse_aridity_rank[,-1] = as.character(apply(p_rmse_aridity_rank[,-1], 2, rank))
et_rmse_aridity_rank = et_rmse_aridity
et_rmse_aridity_rank[,-1] = as.character(apply(et_rmse_aridity_rank[,-1], 2, rank))

p_rmse_elevation_rank = p_rmse_elevation
p_rmse_elevation_rank[,-1] = as.character(apply(p_rmse_elevation_rank[,-1], 2, rank))
et_rmse_elevation_rank = et_rmse_elevation
et_rmse_elevation_rank[,-1] = as.character(apply(et_rmse_elevation_rank[,-1], 2, rank))

p_rmse_cti_rank = p_rmse_cti
p_rmse_cti_rank[,-1] = as.character(apply(p_rmse_cti_rank[,-1], 2, rank))
et_rmse_cti_rank = et_rmse_cti
et_rmse_cti_rank[,-1] = as.character(apply(et_rmse_cti_rank[,-1], 2, rank))

p_rmse_ndvi_rank = p_rmse_ndvi
p_rmse_ndvi_rank[,-1] = as.character(apply(p_rmse_ndvi_rank[,-1], 2, rank))
et_rmse_ndvi_rank = et_rmse_cti
et_rmse_ndvi_rank[,-1] = as.character(apply(et_rmse_ndvi_rank[,-1], 2, rank))

# Plot the new large mosaic of heat maps - 1) Aridity 2) Elevation 3) CTI and 4) NDVI
# Aridity
...
#p_temp_rmse_aridity$RMSE = rescale(p_temp_rmse_aridity$RMSE)
...
#et_temp_rmse_aridity$RMSE = rescale(et_temp_rmse_aridity$RMSE)
data_rmse_aridity = rbind(p_temp_rmse_aridity,et_temp_rmse_aridity)
data_rmse_aridity = cbind(data_rmse_aridity, data.frame(Group="Aridity"))
...
#p_temp_rmse_elevation$RMSE = rescale(p_temp_rmse_elevation$RMSE)
...
#et_temp_rmse_elevation$RMSE = rescale(et_temp_rmse_elevation$RMSE)
data_rmse_elevation = rbind(p_temp_rmse_elevation,et_temp_rmse_elevation)
data_rmse_elevation = cbind(data_rmse_elevation, data.frame(Group="Elevation"))
...
#p_temp_rmse_cti$RMSE = rescale(p_temp_rmse_cti$RMSE)
...
#et_temp_rmse_cti$RMSE = rescale(et_temp_rmse_cti$RMSE)
data_rmse_cti = rbind(p_temp_rmse_cti,et_temp_rmse_cti)
data_rmse_cti = cbind(data_rmse_cti, data.frame(Group="CTI"))
...
#p_temp_rmse_ndvi$RMSE = rescale(p_temp_rmse_ndvi$RMSE)
...
#et_temp_rmse_ndvi$RMSE = rescale(et_temp_rmse_ndvi$RMSE)
data_rmse_ndvi = rbind(p_temp_rmse_ndvi,et_temp_rmse_ndvi)
data_rmse_ndvi = cbind(data_rmse_ndvi, data.frame(Group="NDVI"))

# combine all the data frames for plotting 
data_rmse_all = rbind(data_rmse_aridity, data_rmse_elevation, data_rmse_cti, data_rmse_ndvi)

# create a data_rmse_all_rank for plotting
# Aridity
p_temp_rmse_aridity_rank = melt(data = p_rmse_aridity_rank,id.vars = "P_Dataset")
p_temp_rmse_aridity_rank = cbind(p_temp_rmse_aridity_rank,as.data.frame("Precipitation"))
colnames(p_temp_rmse_aridity_rank) = c("Dataset","Classification","RMSE","Variable")
et_temp_rmse_aridity_rank = melt(data = et_rmse_aridity_rank,id.vars = "ET_Dataset")
et_temp_rmse_aridity_rank = cbind(et_temp_rmse_aridity_rank,as.data.frame("Evapotranspiration"))
colnames(et_temp_rmse_aridity_rank) = c("Dataset","Classification","RMSE","Variable")
data_rmse_aridity_rank = rbind(p_temp_rmse_aridity_rank,et_temp_rmse_aridity_rank)
data_rmse_aridity_rank = cbind(data_rmse_aridity_rank, data.frame(Group="Aridity"))
#remove(p_temp_rmse_aridity,et_temp_rmse_aridity)

# Elevation
p_temp_rmse_elevation_rank = melt(data = p_rmse_elevation_rank,id.vars = "P_Dataset")
p_temp_rmse_elevation_rank = cbind(p_temp_rmse_elevation_rank,as.data.frame("Precipitation"))
colnames(p_temp_rmse_elevation_rank) = c("Dataset","Classification","RMSE","Variable")
et_temp_rmse_elevation_rank = melt(data = et_rmse_elevation_rank,id.vars = "ET_Dataset")
et_temp_rmse_elevation_rank = cbind(et_temp_rmse_elevation_rank,as.data.frame("Evapotranspiration"))
colnames(et_temp_rmse_elevation_rank) = c("Dataset","Classification","RMSE","Variable")
data_rmse_elevation_rank = rbind(p_temp_rmse_elevation_rank,et_temp_rmse_elevation_rank)
data_rmse_elevation_rank = cbind(data_rmse_elevation_rank, data.frame(Group="Elevation"))

# CTI
p_temp_rmse_cti_rank = melt(data = p_rmse_cti_rank,id.vars = "P_Dataset")
p_temp_rmse_cti_rank = cbind(p_temp_rmse_cti_rank,as.data.frame("Precipitation"))
colnames(p_temp_rmse_cti_rank) = c("Dataset","Classification","RMSE","Variable")
et_temp_rmse_cti_rank = melt(data = et_rmse_cti_rank,id.vars = "ET_Dataset")
et_temp_rmse_cti_rank = cbind(et_temp_rmse_cti_rank,as.data.frame("Evapotranspiration"))
colnames(et_temp_rmse_cti_rank) = c("Dataset","Classification","RMSE","Variable")
data_rmse_cti_rank = rbind(p_temp_rmse_cti_rank,et_temp_rmse_cti_rank)
data_rmse_cti_rank = cbind(data_rmse_cti_rank, data.frame(Group="CTI"))

# NDVI
p_temp_rmse_ndvi_rank = melt(data = p_rmse_ndvi_rank,id.vars = "P_Dataset")
p_temp_rmse_ndvi_rank = cbind(p_temp_rmse_ndvi_rank,as.data.frame("Precipitation"))
colnames(p_temp_rmse_ndvi_rank) = c("Dataset","Classification","RMSE","Variable")
et_temp_rmse_ndvi_rank = melt(data = et_rmse_ndvi_rank,id.vars = "ET_Dataset")
et_temp_rmse_ndvi_rank = cbind(et_temp_rmse_ndvi_rank,as.data.frame("Evapotranspiration"))
colnames(et_temp_rmse_ndvi_rank) = c("Dataset","Classification","RMSE","Variable")
data_rmse_ndvi_rank = rbind(p_temp_rmse_ndvi_rank,et_temp_rmse_ndvi_rank)
data_rmse_ndvi_rank = cbind(data_rmse_ndvi_rank, data.frame(Group="NDVI"))

# combine all the data frames for plotting 
data_rmse_all_rank = rbind(data_rmse_aridity_rank, data_rmse_elevation_rank, data_rmse_cti_rank, data_rmse_ndvi_rank)

# add column to data_rmse_all
data_rmse_all$Rank = data_rmse_all_rank$RMSE

# Replace all x axis labels with the required ones
levels(data_rmse_all$Classification) = c("Humid", "Dry sub-humid", "Semi-arid", "Arid", "Hyper-arid",
                                         "< 60", "60-156", "156-360","360-748","748-1287","1287-5300",
                                         "< 9.3", "9.3-10.2", "10.2-11.2","11.2-12.1","12.1-13.1","13.1-47.1",
                                         "< 0.09", "0.09-0.20","0.20-0.38","0.38-0.55","0.55-0.67","0.67-0.86") 


# plot the big mosaic
plot_rmse_all = ggplot()+
                      geom_tile(data = data_rmse_all, aes(x=Classification, y=Dataset,fill=RMSE), color="white")+
                      geom_text(data = data_rmse_all, aes(x=Classification, y=Dataset, label=Rank), color="white", family="Times", size=8)+
                      ...
                      theme(strip.text = element_text(size=8, family="Times", face="bold"))+
                      theme(axis.title = element_blank())
ggsave(plot = plot_rmse_all, filename = "Graphs_For_Manuscript_Revised/Figure_7/HeatMap_All.pdf", width = 190, height = 130, dpi=300, units="mm")
...
#  geom_polygon(data=shp_df, aes(x=long,y=lat,group=group,fill=factor(Land_Cover)),colour="black",size=0.1)+
#scale_fill_manual(values = c("#d11d1d","#b2642c","#37842a","#3faec1","#1f5b93"))+
#  #scale_fill_brewer(type="qual",palette = "Paired")+
...
#  labs(x="Longitude", y="Latitude")
#ggsave(plot_landcover_map,filename = "Output/LandCover_Study_Area_Legend.png", height=5, width=10,dpi=300)
#remove(shp_df)
